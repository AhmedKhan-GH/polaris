...
2. **Copy the exemplar:** `cp -r app/_features/notes app/_features/<name>` — then rename: table, CASL subject (singular, e.g. `'Order'`), topics (`<name>:{userId}` / `<name>:all`), limiter key (`<name>:create:<userId>`), policy names (`<name>_owner_or_self`, `<name>_read_own_topic`), trigger (`broadcast_<name>_change` / `<name>_broadcast`).
3. **TDD, no exceptions:** every behavior starts as one failing test — red (for the right reason) → minimal green → refactor. A test that passes before its implementation lands belongs to an earlier commit or is redundant.
...
8. **Tests at all three tiers:** unit (mocked collaborators), integration (real `app_user` through `startRlsTestDb`/`withUserContext`; live-Supabase suites use the `liveDbGate` pattern), E2E journeys (seeded owner/member, `loginViaSupabase`).
