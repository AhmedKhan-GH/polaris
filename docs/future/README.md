# Future Plans

Long-horizon vision — the directions Polaris is headed *beyond* the current build roadmap.

- **Near-term, scheduled roadmap (F1–F12):** [`HANDBOOK.md`](../../HANDBOOK.md) §6.
- **Active, soon-ish feature designs:** [`docs/superpowers/specs/`](../superpowers/specs/) — org-scoping, observability, progressive-loading, input methods, etc.
- **This folder:** the *further-out* picture that no scheduled feature covers yet.

## What's here

- [`domain-growth-schema.md`](domain-growth-schema.md) — how Polaris grows from an order app into a multi-domain **operations platform** (intake → fulfillment → routing → assets → accounting), with the order as the central "spine." The post-F12 direction.

Add long-range vision docs here as they emerge; promote one into `specs/` (with a date + TDD plan) when its domain actually comes on deck.
