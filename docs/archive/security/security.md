...
Current state mapped against the checklist:

| Concern | Status |
|---|---|
| Server Actions for mutations | ✅ one action (`createOrderAction`) |
| Input validation | ⚪ no inputs currently; add when arguments appear |
| Drizzle query builder | ✅ no raw SQL |
| Domain mapper projects data | ✅ `toOrder` with leak-guard test |
| Structured error logging | ✅ via Pino |
| `.env*` not committed | ✅ gitignored |
| Authentication | ❌ not yet — must gate before public deploy |
| Rate limiting | ❌ not yet — must add before public deploy |
| HTTPS | ⚪ inherited from host when deployed |
| CSP | ❌ not yet — must add before public deploy |
| Default security headers | ❌ not yet — must add before public deploy |
| Database role least-priv | ⚪ review before deploy (Supabase default role review) |
| Migrations in deploy pipeline | ❌ currently `npm run dev` only |
| `LOG_LEVEL=info` in prod | ⚪ inherit from env on deploy |

✅ done, ⚪ N/A or handled-by-platform, ❌ outstanding before public deploy.

The outstanding items are the §3 pre-deploy list. None of the always-on rules are being violated by current code.

---

## TL;DR

Follow §1 while coding. Follow §2 when adding features. Run §3 before any public URL. Ignore §4 entirely. Apply §5 to decide between Server Action and API Route on every new mutation. Refer to §6 only when something unfamiliar comes up. Check §7 to know where Polaris currently sits.
