...
// USER-CONFIRMED canonical hexes and the source logo geometry, so neither can
// drift silently. No app import — foundation never depends on a feature (Iron
// Rule 1); the ratio derivation is proven in the brand feature's own tests.
...
describe('branding brand tokens', () => {
  ...
  it('references the corrected lockup asset served from /public (~2.8:1)', () => {
    expect(branding.lockup.src).toBe('/zeefoods_lockup.svg');
    expect(branding.lockup.alt.length).toBeGreaterThan(0);
    expect(branding.lockup.width / branding.lockup.height).toBeCloseTo(2.8, 1);
  });

  it('records the source logo geometry so the displayed ratios stay derived', () => {
