 ...
 * export table, manifests, private internals, and the index.ts source
 * verbatim as the contract. Mirrors the guide's look (same tokens/base CSS,
 * dark code figures) and links back to it.
 */
export function renderFeaturePageHtml(surface, indexSource) {
  const { feature, publicExports, manifests, privateFiles } = surface;
  const lead = parseIndexDocLead(indexSource);
  const exportRows = publicExports
    .map(
      (e) =>
        `      <tr><td><code>${escapeHtml(e.name)}</code>${e.typeOnly ? ' <em>(type)</em>' : ''}</td><td><code>${escapeHtml(e.from)}</code></td></tr>`,
    )
    .join('\n');
...
<header class="hero wrap prose">
  ...
  <h1 style="font-size: var(--text-3xl); margin-top: var(--space-2)">${escapeHtml(feature)}</h1>
  <p class="lead">${escapeHtml(lead)}</p>
  <p class="muted" style="font-size: var(--text-sm); margin-top: var(--space-4)">
  Generated from <code>app/_features/${escapeHtml(feature)}/index.ts</code> —
...
<section class="doc wrap prose">
  ...
  <table>
    <tr><th>Export</th><th>Source module</th></tr>
${exportRows}
  </table>
</section>
...
function main() {
  ...
  const surfaces = readdirSync(featuresRoot, { withFileTypes: true })
    ...
    .map((feature) => {
      ...
      const indexSource = files.includes('index.ts')
        ...
        : '';
      return { feature, indexSource, ...classifyFeatureFiles(files, indexSource) };
    });
  ...
  for (const s of surfaces) {
    writeFileSync(
      ...
      renderFeaturePageHtml(s, s.indexSource),
    );
  }
  console.log(
