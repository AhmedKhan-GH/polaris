...
<section class="doc wrap prose">
  <h2>At a glance</h2>
  <div class="glance">
    <div class="card"><h3>${publicExports.length}</h3><p>public export${publicExports.length === 1 ? '' : 's'} — the entire dev API</p></div>
    <div class="card"><h3>${manifests.length}</h3><p>manifest${manifests.length === 1 ? '' : 's'} on the registry seam</p></div>
    <div class="card"><h3>${privateFiles.length}</h3><p>private internal${privateFiles.length === 1 ? '' : 's'} — unreachable from outside</p></div>
  </div>
